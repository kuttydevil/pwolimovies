...
function createMediaDetailsCard(media) {
    ...
    const ratingValue = media.vote_average || "N/A";
    const ratingCount = media.vote_count || null; // Get vote_count
    const ratingElement = `
        <span itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating">
          <span itemprop="ratingValue">${ratingValue}</span>/<span itemprop="bestRating">10</span>
          ${ratingCount !== null ? `<meta itemprop="ratingCount" content="${ratingCount}" />` : ''}
        </span>`;
